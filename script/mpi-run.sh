...
for (( i="0" ; $i<$iterations ; i=$i+1 )); do
    mpirun --stdin 0 ./bin/mpi-skyline < ./input/${input_name}*.in > ./output/mpi/${input_name}.out 2>./temp
    diff ./output/mpi/${input_name}.out ./output/serial/${input_name}*.out;

    time_string=`tail -n 1 ./temp`
    echo "Iteration #$(($i+1)), ${time_string}" | tee -a ./bench/mpi/${input_name}.data
    time_sum=$(echo ${time_sum} + ${time_string:19} | bc)
done

echo "Average execution time (s): $(echo "scale=6; ${time_sum} / ${iterations}" | bc)" | tee -a ./bench/mpi/${input_name}.data
